...
class GeneratorLosses():
    ...
    def compute_style(gen_output_image, ground_truth_image):
    ...
    def style_loss(gen_output_image, ground_truth):
      ...
      l_style_gen = 0.
      for i in range(len(features_ground_truth)):
        loss = compute_style(features_output_image[i], features_ground_truth[i])
        l_style_gen += loss
      return l_style_gen

    def total_variation_row_loss(complete_image, region):
      x_var = tf.reduce_sum(
    ...
    def total_variation_col_loss(complete_image, region):
      y_var = tf.reduce_sum(
    ...
    def total_variation_loss(complete_image, mask):
      ...
      x_var = total_variation_row_loss(complete_image, region)
      y_var = total_variation_col_loss(complete_image, region)

      loss = x_var + y_var
      return loss

    def gt_loss(dis_real):
     return tf.square(tf.reduce_mean(dis_real))

    def generator_loss(output_gen, ground_truth, complete_image, mask, dis_fake, dis_real):
      loss = per_pixel_loss(output_gen, ground_truth, mask, alpha=6.)
      percep_loss = perceptual_loss(complete_image, ground_truth,output_gen)
      ...
      generator_loss = total_loss + (0.001 * gsn_loss(dis_fake)) + (0.001 * gt_loss(dis_real))
